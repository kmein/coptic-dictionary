...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" version="2.0">
  ...
  <xsl:template name="encode-string"> <xsl:param name="s" select="''"/> "<xsl:value-of select="replace(replace($s, '&quot;', '\\&quot;'), '&#xA;', ' ')"/>" </xsl:template>

  <xsl:template name="encode-array"> <xsl:param name="element" select="()"/> (<xsl:for-each select="$element"> <xsl:apply-templates select="."/> <xsl:if test="position() != last()">, </xsl:if> </xsl:for-each>) </xsl:template>

  <xsl:template match="tei:note | tei:gram | tei:subc | tei:ref[@type='Greek'] | tei:quote[@xml:lang]"> <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="."/> </xsl:call-template> </xsl:template>


  <xsl:template match="tei:etym">( notes: <xsl:call-template name="encode-array"> <xsl:with-param name="element" select="tei:note"/> </xsl:call-template> , references: <xsl:call-template name="encode-array"> <xsl:with-param name="element" select="tei:xr/tei:ref"/> </xsl:call-template> , equivalentGreek: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="tei:ref[@type=Greek]"/> </xsl:call-template> , greekLemma: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="tei:ref[@type='greek_lemma::grl_meaning']"/> </xsl:call-template> , greekMeaning: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="tei:ref[@type='greek_lemma::grl_meaning']"/> </xsl:call-template> , greekPartOfSpeech: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="tei:ref[@type='greek_lemma::grl_pos']"/> </xsl:call-template> , greekReference: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="tei:ref[@type='greek_lemma::grl_ref']"/> </xsl:call-template>)</xsl:template>

  <xsl:template match="tei:ref"> ( target: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="@tei:target"/> </xsl:call-template> , reference: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="."/> </xsl:call-template>) </xsl:template>

  <xsl:template match="tei:cit"> ( bibl: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="tei:bibl"/> </xsl:call-template> , type: <xsl:call-template name="encode-string"> <xsl:with-param name="s" select="@type"/> </xsl:call-template> , quoteDE: <xsl:call-template name="encode-array"> <xsl:with-param name="element" select="tei:quote[@xml:lang='de']"/> </xsl:call-template> , quoteEN: <xsl:call-template name="encode-array"> <xsl:with-param name="element" select="tei:quote[@xml:lang='en']"/> </xsl:call-template> , quoteFR: <xsl:call-template name="encode-array"> <xsl:with-param name="element" select="tei:quote[@xml:lang='fr']"/> </xsl:call-template>) </xsl:template>
